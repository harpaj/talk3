...
#For New Data
df_pron_count_test_set = pd.DataFrame((list(sn.count_pronouns(x)) for x in df_test_set["sentence"].map(str) + "'\n'" + df_test_set["after"].map(str) + "'\n'" + df_test_set["third"].map(str)), columns=['first_per_pron', 'sec_per_pron', 'Third_per_pron', 'personal_pron', 'total_pron', 'sing_proper_noun'])
...
global ls_result
ls_result=[]
features_k = df_train_set.columns[7:]
df_csv = pd.DataFrame(columns=['Features','Sentiment-Accuracy','Factual Accuracy'])
#features_k = pd.DataFrame(columns=['polarity','subjectivity'])
#print(features_k.columns)

#print(features_k)

for L in range(0, len(features_k)+1):
    for subset in combinations(features_k, L):
        ...
        if len(features)>0:
            ...
            res = [list(features),sentiment_accuracy,factual_accuracy]
            print(res)
